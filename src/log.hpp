...
public:
	...
	void static OnServerEvent(int Event) {
		...
		// Init Log Stream
		string log_file = server.LogDirectory + "/server.log";
		...
		case CFtpServer::STOP_ACCEPTING:
			log << ts << " - Server stopped accepting incoming connections !\r\n";
			break;

		case CFtpServer::MEM_ERROR:
			log << ts << " - Warning, the CFtpServer class could not allocate memory !\r\n";
			break;

		case CFtpServer::THREAD_ERROR:
			log << ts << " - Warning, the CFtpServer class could not create a thread !\r\n";
			break;

		case CFtpServer::ZLIB_VERSION_ERROR:
			log << ts << " - Warning, the Zlib header version differs from the Zlib library version !\r\n";
			break;

		case CFtpServer::ZLIB_STREAM_ERROR:
			log << ts << " - Warning, error during compressing/decompressing data !\r\n";
			break;

		case CFtpServer::HDFS_CONNECT_ERROR:
			log << ts << " - Warning, unable to connect to HDFS Cluster !\r\n";
			break;
	...
	void static OnUserEvent(int Event, CFtpServer::CUserEntry *pUser,
		...
		// Init Log Stream
		string log_file = server.LogDirectory + "/user.log";
		ofstream log(log_file.c_str(), ios::app);
	...
	void static OnClientEvent(int Event, CFtpServer::CClientEntry *pClient,
		...
		// Init Log Stream
		string log_file = server.LogDirectory + "/client.log";
		ofstream log(log_file.c_str(), ios::app);
