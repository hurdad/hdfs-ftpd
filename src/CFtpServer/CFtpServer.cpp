...
CFtpServer::CFtpServer(void) {
	...
	HDFSConfig.HDFSNameNodeHost = "127.0.0.1";
	HDFSConfig.HDFSNameNodePort = 50010;
...
bool CFtpServer::StartListening(unsigned long ulAddr, unsigned short int usPort) {
	...
	FtpServerLock.Enter();
	{
		...
		if (ListeningSock != INVALID_SOCKET
				&& setsockopt(ListeningSock, SOL_SOCKET, SO_REUSEADDR, (char *) &on,
						sizeof(on)) != SOCKET_ERROR

						&& bind(ListeningSock, (struct sockaddr *) &sin,
...
void*
CFtpServer::StartAcceptingEx(void *pvParam)
...
char*
CFtpServer::CClientEntry::BuildPath(char* pszAskedPath, char **pszVirtualPath /* = NULL */) {
	if (!pUser)
	...
	if (pszBuffer) {
		...
		if (pszVirtualP) {
			...
			if (snprintf(pszBuffer, MAX_PATH + 4, "%s/%s", pUser->GetStartDirectory(), pszVirtualP)
					> 0) {
...
char*
CFtpServer::CClientEntry::BuildVirtualPath(char* pszAskedPath) {
	char *pszBuffer = new char[ MAX_PATH + 4];
	if (pszBuffer) {
		...
		if (pszAskedPath) {
			if (pszAskedPath[0] == '/') {
				...
				if (snprintf(pszBuffer, MAX_PATH + 4, "%s/%s", GetWorkingDirectory(), pszAskedPath)
						> 0) {
					SimplifyPath(pszBuffer);
					if (strlen(pszBuffer) > MAX_PATH)
						bErr = true;
...
CFtpServer::CUserEntry *
CFtpServer::AddUser(const char *pszLogin, const char *pszPass, const char *pszStartDir) {
	if (pszLogin && pszStartDir && strlen(pszLogin) <= MaxLoginLen
		...
		if (pUser) {
			...
			free(pszSDEx);
			if (pszPass)
				strcpy(pUser->szPassword, pszPass);
...
bool CFtpServer::DeleteUser(CFtpServer::CUserEntry *pUser) {
	if (pUser != NULL) {
		...
		UserListLock.Enter();
		{
			...
			if (pUser->pPrevUser)
				pUser->pPrevUser->pNextUser = pUser->pNextUser;
			if (pUser->pNextUser)
				pUser->pNextUser->pPrevUser = pUser->pPrevUser;
			if (pFirstUser == pUser)
				pFirstUser = pUser->pNextUser;
			if (pLastUser == pUser)
				pLastUser = pUser->pPrevUser;
...
CFtpServer::AddClient(SOCKET Sock, struct sockaddr_in *Sin) {
		...
		if (pClient) {
			...
			ClientListLock.Enter();
			{
				if (pLastClient) {
					...
					pFirstClient = pLastClient = pClient;
			}
			ClientListLock.Leave();
...
CFtpServer::CUserEntry *
CFtpServer::SearchUserFromLogin(const char *pszLogin) {
	if (pszLogin) {
...
CFtpServer::CClientEntry::~CClientEntry() {
	...
	if (pPrevClient)
		pPrevClient->pNextClient = pNextClient;
	if (pNextClient)
		pNextClient->pPrevClient = pPrevClient;
	if (pFtpServer) {
...
void *
CFtpServer::CClientEntry::Shell(void *pvParam) {
	...
	for (;;) { // Main Loop
		...
		if (nCmd == CMD_NONE) {
			...
			if (!pszCmdArg) {
				...
				pFtpServer->UserListLock.Enter();
				{
					...
					if (pClient->pUser && pClient->pUser->bIsEnabled
							&& !*pClient->pUser->szPassword) {
						pClient->LogIn();
					...
					if (pClient->pUser && pClient->pUser->bIsEnabled) { // User is valid & enabled
						if ((pszCmdArg && !strcmp(pszCmdArg, pClient->pUser->szPassword))
							...
							if (pClient->pUser->uiMaxNumberOfClient == 0 //  0 = Unlimited
								...
								pClient->SendReply(
										"421 Too many users logged in for this account.");
						} else if (!pszCmdArg)
							pClient->SendReply("501 Invalid number of arguments.");
					}
				}
			...
			if (pFtpServer->GetMaxPasswordTries()
					&& pClient->nPasswordTries >= pFtpServer->GetMaxPasswordTries()) {
		...
		else if( nCmd == CMD_SITE )
		{
			...
			if (pClient->eStatus == WAITING && pszCmdArg
					&& sscanf(pszCmdArg, "%u,%u,%u,%u,%u,%u", &iIp1, &iIp2, &iIp3, &iIp4, &iPort1,
							&iPort2) == 6 && iIp1 <= 255 && iIp2 <= 255 && iIp3 <= 255
					&& iIp4 <= 255 && iPort1 <= 255 && iPort2 <= 255
					&& (iIp1 | iIp2 | iIp3 | iIp4) != 0 && (iPort1 | iPort2) != 0) {
				...
				if (!pFtpServer->IsFXPEnabled()
						|| (ulPortIp == pClient->ulClientIP
								|| pClient->ulClientIP == inet_addr("127.0.0.1"))) {
			...
			if (pClient->eStatus == WAITING) {
				...
				int on = 1;
				if (pClient->DataSock != INVALID_SOCKET && setsockopt(pClient->DataSock, SOL_SOCKET,
			...
			if(pszPath && (fileAttr = hdfsGetPathInfo(pClient->fs, pszPath)) != NULL) {
			...
			if (pszCmdArg) {
				...
				if(pszPath && (fileAttr = hdfsGetPathInfo(pClient->fs, pszPath)) != NULL && fileAttr->mKind == kObjectKindDirectory) {
			...
			if (pszCmdArg) {
				...
				struct tm *t;
				if(pszPath && (fileAttr = hdfsGetPathInfo(pClient->fs, pszPath)) != NULL && (t = gmtime((time_t *) &(fileAttr->mLastMod)))) {
					pClient->SendReply2("213 %04d%02d%02d%02d%02d%02d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
				} else
					pClient->SendReply("550 No such file or directory.");
			} else
				pClient->SendReply("501 Invalid number of arguments.");
			...
			if (pszCmdArg) {
				...
				if(pszPath && (fileAttr = hdfsGetPathInfo(pClient->fs, pszPath)) != NULL && fileAttr->mKind == kObjectKindFile) {
			...
			if (pszCmdArg) {
				pszPath = pClient->BuildPath(pszCmdArg);
				if(pszPath && (fileAttr = hdfsGetPathInfo(pClient->fs, pszPath)) != NULL && fileAttr->mKind == kObjectKindFile) {
					if (hdfsDelete(pClient->fs, pszPath, 0) != -1) {
			...
			if (pszCmdArg) {
				if (pClient->szRenameFromPath) {
					pszPath = pClient->BuildPath(pszCmdArg);
					if (pszPath && hdfsRename(pClient->fs, pClient->szRenameFromPath, pszPath) == 0) {
	...
	for (;;) {
		...
		if (!FD_ISSET(CtrlSock, &fdRead)) {
			...
			if ((!IsLogged() && pFtpServer->GetNoLoginTimeout())
				...
				pFtpServer->OnClientEventCb(IsLogged() ? NO_TRANSFER_TIMEOUT : NO_LOGIN_TIMEOUT,
						this);
				return false;
			} else
				continue;
...
int CFtpServer::CClientEntry::ParseLine() {
	...
	case 'A':
		switch (sCmdBuffer[1]) {
		case 'B':
			if (!strcmp(sCmdBuffer + 2, "OR"))
				return CMD_ABOR;
			break;
		case 'L':
			if (!strcmp(sCmdBuffer + 2, "LO"))
				return CMD_ALLO;
			break;
		case 'P':
			if (!strcmp(sCmdBuffer + 2, "PE"))
				return CMD_APPE;
			break;
		}
		break;
	case 'C':
		switch (sCmdBuffer[1]) {
		case 'D':
			if (!strcmp(sCmdBuffer + 2, "UP"))
				return CMD_CDUP;
			break;
		case 'L':
			if (!strcmp(sCmdBuffer + 2, "NT"))
				return CMD_CLNT;
			break;
		case 'W':
		...
		break;
	case 'D':
		if (!strcmp(sCmdBuffer + 1, "ELE"))
			return CMD_DELE;
		break;
	case 'F':
		if (!strcmp(sCmdBuffer + 1, "EAT"))
			return CMD_FEAT;
		break;
	case 'H':
		if (!strcmp(sCmdBuffer + 1, "ELP"))
			return CMD_HELP;
		break;
	case 'L':
		if (!strcmp(sCmdBuffer + 1, "IST"))
			return CMD_LIST;
		break;
	case 'M':
		switch (sCmdBuffer[1]) {
		case 'D':
			if (!strcmp(sCmdBuffer + 2, "TM"))
				return CMD_MDTM;
			break;
		case 'K':
			if (!strcmp(sCmdBuffer + 2, "D"))
	...
	case 'N':
		switch (sCmdBuffer[1]) {
		case 'L':
			if (!strcmp(sCmdBuffer + 2, "ST"))
				return CMD_NLST;
			break;
		case 'O':
			if (!strcmp(sCmdBuffer + 2, "OP"))
				return CMD_NOOP;
			break;
		}
		break;
	case 'O':
		if (!strcmp(sCmdBuffer + 1, "PTS"))
			return CMD_OPTS;
		break;
	case 'P':
		switch (sCmdBuffer[1]) {
	...
	case 'R':
		...
		case 'E':
			switch (sCmdBuffer[2]) {
			case 'S':
				if (!strcmp(sCmdBuffer + 3, "T"))
					return CMD_REST;
				break;
			case 'T':
				if (!strcmp(sCmdBuffer + 3, "R"))
					return CMD_RETR;
				break;
			}
			break;
	...
	case 'S':
		...
		break;
	case 'T':
		if (!strcmp(sCmdBuffer + 1, "YPE"))
			return CMD_TYPE;
		break;
	case 'U':
		if (!strcmp(sCmdBuffer + 1, "SER"))
			return CMD_USER;
		break;
	case 'X':
		switch (sCmdBuffer[1]) {
		case 'C':
			switch (sCmdBuffer[2]) {
			case 'U':
				if (!strcmp(sCmdBuffer + 3, "P"))
					return CMD_XCUP;
				break;
			case 'W':
				if (!strcmp(sCmdBuffer + 3, "D"))
					return CMD_XCWD;
				break;
			}
			break;
		case 'M':
			if (!strcmp(sCmdBuffer + 2, "KD"))
				return CMD_XMKD;
		case 'R':
			if (!strcmp(sCmdBuffer + 2, "MD"))
				return CMD_XRMD;
		case 'P':
			if (!strcmp(sCmdBuffer + 2, "PW"))
				return CMD_XPWD;
		}
		break;
	}
...
bool CFtpServer::CClientEntry::OpenDataConnection(int nCmd) {
	...
	if (DataSock != INVALID_SOCKET
			&& !setsockopt(DataSock, SOL_SOCKET, (nCmd == CMD_STOR) ? SO_RCVBUF : SO_SNDBUF,
...
bool CFtpServer::CClientEntry::SafeWrite(hdfsFile hFile, char *pBuffer, int nLen) {
	int wl = 0, k;
	while (wl != nLen) {
		k = hdfsWrite(fs, hFile, pBuffer + wl, nLen - wl);
		if (k < 0)
			return false;
		wl += k;
	}
...
void*
CFtpServer::CClientEntry::StoreThread(void *pvParam)
...
void*
CFtpServer::CClientEntry::RetrieveThread(void *pvParam)

{
	...
	theSize = pFtpServer->GetHDFSBufferSize();
	...
	if (hFile >= 0) {
		if (pTransfer->RestartAt == 0 || (pTransfer->RestartAt > 0
			...
			while (pClient->DataSock != INVALID_SOCKET
					&& (BlockSize = hdfsRead(pClient->fs, hFile, pBuffer, uiBufferSize)) >= 0) {
#ifdef CFTPSERVER_ENABLE_ZLIB
				if (pTransfer->eDataMode == ZLIB)
				{
...
bool CFtpServer::CClientEntry::AddToListBuffer(DataTransfer_t *pTransfer, char *pszListLine,
		int nLineLen, char *pBuffer, unsigned int *nBufferPos, unsigned int uiBufferSize) {
...
void*
CFtpServer::CClientEntry::ListThread(void *pvParam) {
	CFtpServer::CClientEntry *pClient = (CFtpServer::CClientEntry*) pvParam;
	CFtpServer *pFtpServer = pClient->pFtpServer;
	struct DataTransfer_t *pTransfer = &pClient->CurrentTransfer;
				...
				do {
					if (pClient->CurrentTransfer.nCmd == CMD_NLST) {
						...
						if (fi->pszName[0] != '.' || pTransfer->opt_a) {
							if (!pClient->AddToListBuffer(pTransfer, fi->pszName,
									strlen(fi->pszName), pBuffer, &nBufferPos, uiBufferSize))
								break;
						}
